...
# from gym_citynav.utils.traffic_generator import TrafficGenerator
import cv2
import csv
import re
import json
...
class TrafficEnv(gym.Env):
    ...
    def __init__(self, action_config="action_config.json", render_mode=None, observation_type="ground_truth", record_video=False, log_dir=None, reward_setting=None, resolution=(320, 240), port=9000):
        ...
        self.traffic_controller.init_communicator(self.communicator)
        ...
        self.current_red_light_violation = 0
        self.total_red_light_violation = 0
    ...
    def _check_red_light_violation(self):
        def is_point_in_rectangles(current_pos, rectangles):
        ...
        agent_pos = np.array([self._agent_location[0], self._agent_location[1]])
        positions = np.array([[ts.position.x, ts.position.y] for ts in traffic_signals])
        distances = np.linalg.norm(positions - agent_pos, axis=1)
        sorted_indices = np.argsort(distances)
        if distances[sorted_indices[0]] < 2000:
            closest_signal = [traffic_signals[i] for i in sorted_indices[0:4]]
            xs = [signal.position.x for signal in closest_signal]
            ys = [signal.position.y for signal in closest_signal]
            x_min = min(xs)
            y_min = min(ys)
            x_max = max(xs)
            y_max = max(ys)

            cross_walk_width = 1500
            rectangles = [(x_min, y_min, x_max, y_max),
                        (x_min-cross_walk_width, y_min, x_min, y_max),
                        (x_max, y_min, x_max+cross_walk_width, y_max),
                        (x_min, y_min-cross_walk_width, x_max, y_min),
                        (x_min, y_max, x_max, y_max+cross_walk_width)]
            if is_point_in_rectangles(agent_pos, rectangles):
                if closest_signal[0].get_state()[1].value == 'pedestrian_red':
                    return 1
                elif closest_signal[0].get_state()[1].value == 'pedestrian_green':
                    return 0
                else:
                    raise ValueError("Invalid traffic signal state")
            else:
                return 0
        else:
            return 0
    ...
    def _get_infor(self):
        ...
        self.total_red_light_violation += self.current_red_light_violation
        ...
        return {
            ...
            "total_red_light_violation": self.total_red_light_violation
        }
    ...
    def _get_reward(self):
        if self._get_success():
            ...
            collision_penalty = self.current_human_collision * self.human_collision_penalty + self.current_object_collision * self.object_collision_penalty + self.current_building_collision * self.building_collision_penalty + self.current_vehicle_collision * self.vehicle_collision_penalty
    ...
    def reset(self, options=None, seed=None):
        ...
        if options is not None:
            ...
            if "traffic_json" in options:
                self.traffic_controller.stop_simulation()
                num_vehicles = options["traffic_json"]["num_vehicles"]
                num_pedestrians = options["traffic_json"]["num_pedestrians"]
                center_point = Vector(options["traffic_json"]["center_point"]["x"], options["traffic_json"]["center_point"]["y"])
                roads = misc.get_settingpath(options["traffic_json"]["roads"])
                self.traffic_controller.reset(num_vehicles, num_pedestrians, roads, center_point) 
                print("Reset traffic")
            ...
            if "agent_json" in options:
                ...
                if "traffic_json" in options:
                    self.traffic_controller.spawn_objects_in_unreal_engine()
        ...
        header = ["step", "timestamp", "action", "action_meaning", "agent_x", "agent_y", "agent_z", "agent_rotation", 
                 "target_x", "target_y", "moving_distance", "human_collision", "object_collision", "building_collision", "vehicle_collision", "red_light_violation", "subtask_success", "success"]
        ...
        initial_state = ["Initial Position", "", "", "", f"{self._agent_location[0]:.2f}", f"{self._agent_location[1]:.2f}", 
                        f"{self._agent_location[2]:.2f}", f"{self._agent_rotation[1]:.2f}", "", "", "0", "0", "0", "0", "0", "0", "0"]
    ...
    def step(self, action: int) -> tuple[np.ndarray, float, bool, bool, dict]:
        ...
        if action == -1:
            ...
            if self.record_video:
                ...
                time.sleep(performing_time+0.1)
        ...
        self.current_red_light_violation = self._check_red_light_violation()
        ...
        if self.trajectory_writer is not None:
            self._write_trajectory(self.trajectory_writer, 
                                ...
                                self.current_red_light_violation,
    ...
    def traffic_simulation(self, exit_event):
        ...
        try:
            ...
            self.traffic_controller.pedestrian_manager.set_pedestrians_max_speed(self.communicator)
            self.traffic_controller.intersection_manager.set_traffic_signal_duration(self.communicator)

            while not exit_event.is_set():
                self.traffic_controller.update_states()
                self.traffic_controller.vehicle_manager.update_vehicles(self.communicator, self.traffic_controller.intersection_manager, self.traffic_controller.pedestrians)
                ...
                time.sleep(self.traffic_controller.dt)
    ...
    def _write_trajectory(self, trajectory_writer, steps, action_start_time, action, agent_location, agent_rotation, target_location, moving_distance, human_collision, object_collision, building_collision, vehicle_collision, red_light_violation, current_subtask, success):
