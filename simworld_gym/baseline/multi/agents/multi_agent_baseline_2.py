...
class MultiAgentFirstBaselineAgent(BaseAgentBuffer):
    """Multi-agent baseline: Agent 1 navigates with LLM, Agent 0 does robot detection."""
        ...
        while not self.is_finished:
            if self.agent_controller.return_agent_availability():
                ...
                if action_id not in self.VALID_ACTIONS:
                    print(f"[Agent-{self.self_index}] Invalid action, fallback to NOOP")
                    continue
    ...
    def get_last_llm_output(self):
        """Get the last LLM output for debugging."""
    ...
    def _generate_action(self, obs):
        ...
        if self.self_index == 1:
            ...
            sys_prompt = (
                "You are a navigation robot in a 3D city grid.\n"
                ...
                "Return 100 only when you see a robot dog with yellow body and black legs.\n"
            ...
            raw = self.gpt.chat(sys_prompt, usr_prompt, image=img, max_tokens=256)
            print(cur_sub)
            ...
            sys_prompt = (
                ...
                "Return ONLY an integer: 100 if you see a robot dog, -100 otherwise."
            )
